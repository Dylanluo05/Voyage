...
export default function ExpenseSplitPanel({ trip, currentUserId, onUpdate }: ExpenseSplitPanelProps) {
    ...
    function recalculateSplits(newAmount: number, splits: typeof form.splits) {
        const included = splits.filter(s => s.included).length;
        const share = included > 0 ? newAmount / included : 0;
    ...
    async function handleAdd(e: React.FormEvent) {
        ...
        try {
            ...
            onUpdate(updated);
            setForm({ title: '', amount: 0, splits: members.map(m => ({ ...m, included: true, amount: 0 })) });
            setShowForm(false);
        } catch (err) {
    ...
    return (
        <section className="card expense-panel">
            <div className="expense-header-row">
                ...
                <button type="button" className="ghost small-btn" onClick={() => setShowForm(f => !f)}>
            ...
            {showForm && (
                <form onSubmit={handleAdd} className="expense-form">
                            ...
                            <input
                                ...
                                onChange={e => {
                                    ...
                                    setForm(prev => ({
                                        ...prev,
                                        amount: newAmount,
                                        splits: recalculateSplits(newAmount, prev.splits),
                                    }));
                                }}
                                required
                            />
                                ...
                                <input
                                    ...
                                    onChange={() => {
                                        setForm(prev => {
                                            ...
                                            return { ...prev, splits: recalculateSplits(prev.amount, updated) };
                                        });
                                    }}
                                />
                    ...
                    <button type="submit" disabled={saving || !form.title || form.amount <= 0}>
            ...
            {trip.expenses.length > 0 ? (
                ...
                !showForm && <p className="muted small">No expenses yet. Add one to start splitting costs.</p>
            )}
        </section>
    );
}
